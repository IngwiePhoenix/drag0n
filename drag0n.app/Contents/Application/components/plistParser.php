...
class plistParser extends XMLReader
{
    ...
    private function process() {
		...
		if(count($plist) == 1 && $plist[0]) {
			...
			$this->plist = $plist[0];
			return $this; // pro method chaining!
		} else {
			$this->plist = $plist;
			return $this; // pro method chaining!
		}
	...
	public function toDrag0n() {
		$plistArray = $this->plist;
		$output = new stdClass;
		$output->INFO = (object)$plistArray['INFO'];
		$output->INFO->icon = $plistArray['INFO']['icon'];
		$output->INFO->deps = (object)$plistArray['INFO']['deps'];
		$output->INFO->collision=array();
		foreach($plistArray['INFO']['collision'] as $key=>$val) $output->INFO->collision[$key]=(object)$val; 
		$output->INFO->instDeps = $plistArray['INFO']['instDeps'];
		$output->INFO->uninstColls = $plistArray['INFO']['uninstColls'];
		$output->DATA=new stdClass;
		foreach($plistArray['DATA'] as $key=>$value) {
			$output->DATA->$key=(object)$value;
			foreach($value as $skey=>$sval) {
				if(is_array($sval))
					$output->DATA->$key->$skey=(object)$sval;
				else
					$output->DATA->$key->$skey=$sval;				
			}
		}
		$output->DATA->ADDITINAL=array();
		foreach($plistArray['DATA']['ADDITIONAL'] as $key=>$value) {
			if(!is_array($value)) {
				$output->DATA->ADDITINAL[$key]=(object)$value;
			} else {
				$output->DATA->ADDITINAL[$key]=$value;
			}
		}
		return $output;
	}
